...
from utils import get_kst, decode_image_safe, SchoolPDF
import io
import base64
                ...
                for i, row in data.iterrows():
                    with st.expander(f"📌 {row['제출일시']} - {row['이름']} 학생"):
                        try:
                            ...
                            admin_pdf = pdf_gen.generate_report(r_d, decode_image_safe(row.get('보호자서명', "")), 
                                                                    decode_image_safe(row.get('학생서명', "")), 
                                                                    decode_image_safe(ev_chunks), fixed_info)
