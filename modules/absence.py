...
import pandas as pd
from utils import get_kst, process_multiple_images, process_sig, decode_multiple_images_safe, send_discord_notification, SchoolPDF
import io
import base64
...
def show_page(conn, student_options, fixed_info, paths):
    ...
    st.info(f"선택 기간 내 평일은 **{calc_days}일**입니다.")
            ...
            if not g_name or calc_days == 0:
                ...
                try:
                    ...
                    st.session_state.pdf_data = pdf_gen.generate_report(
                        rep_data, io.BytesIO(base64.b64decode(g_b64)), io.BytesIO(base64.b64decode(s_b64)), ev_ios, fixed_info
                    )
