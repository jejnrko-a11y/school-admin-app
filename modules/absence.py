...
def show_page(conn, user, fixed_info, paths):
    ...
    if st.session_state.get('submitted'):
        ...
        st.download_button("📄 PDF 다운로드", data=st.session_state.pdf_data, 
    ...
    if st.session_state.preview_done:
        ...
        if st.button("🚀 2단계: 위 내용으로 최종 제출", use_container_width=True):
            try:
                ...
                send_discord_notification(f"🔔 [결석계 제출] {st.session_state.student_name}")
                st.session_state.pdf_data = st.session_state.temp_pdf_data
                st.session_state.submitted = True
                st.rerun()
            except Exception as e: st.error(f"제출 실패: {e}")
