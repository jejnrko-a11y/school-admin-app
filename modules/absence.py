...
def show_page(conn, student_options, fixed_info, paths):
    ...
    with st.form("absence_form"):
        ...
        with sig_col1:
            st.write("보호자 서명")
            g_canvas = st_canvas(height=100, width=200, stroke_width=3, key="g_sig", background_color="rgba(0,0,0,0)", update_streamlit=True)
        with sig_col2:
            st.write("학생 서명")
            s_canvas = st_canvas(height=100, width=200, stroke_width=3, key="s_sig", background_color="rgba(0,0,0,0)", update_streamlit=True)
            ...
            if not g_name or calc_days == 0 or not reason_detail:
                ...
                try:
                    ...
                    row_dict = {
                        ...
                        "학생서명": f"'{s_b64}" if s_b64 else "", "보호자서명": f"'{g_b64}" if g_b64 else ""
                    }
