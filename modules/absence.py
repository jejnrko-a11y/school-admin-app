...
def show_page(conn, user, fixed_info, paths):
    if st.session_state.get('submitted'):
        st.title("✅ 제출 완료")
        st.download_button(
            label="📄 통합 결석계 PDF 다운로드",
            data=st.session_state.pdf_data,
            file_name=f"결석계_{user['name']}.pdf",
            mime="application/pdf",
            use_container_width=True
        )
        if st.button("추가 작성하기"):
            st.session_state.submitted = False
            st.rerun()
        return

    st.title("📝 결석신고서 작성")
    st.info(f"작성자: {user['name']} ({user['num']}번)")

    c1, c2 = st.columns(2)
    start_d = c1.date_input("결석 시작일")
    end_d = c2.date_input("결석 종료일")
        ...
        if st.form_submit_button("✅ 결석신고서 제출"):
            if not g_name or calc_days == 0:
                ...
                try:
                    ...
                    send_discord_notification(f"🔔 [결석계] {user['name']}({user['num']}번) / {per_str}")
