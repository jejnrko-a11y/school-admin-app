...
def show_page(conn, user):
    ...
    # --- 3. [핵심] 세션 상태(Session State) 관리 (시트 참조 안 함) ---
    # 앱을 새로고침하거나 메뉴를 이동해도 배치가 유지되도록 메모리에 저장합니다.
    if 'current_layout' not in st.session_state:
        ...
        s_idx = 0
        for c in range(columns_count - 1, -1, -1): # 5분단부터 채우기
    ...
    st.markdown("""
        ...
        @media print {
            ...
            /* ★ 좌석 칸 높이 120% 확대 (145px) */
            .seat-card {
                min-height: 145px !important; 
                border: 2px solid #000 !important;
                box-shadow: none !important;
            }
            /* ★ 이름 및 번호 크기 1.6배 확대 (38px) */
            .seat-name { 
            ...
            .seat-x { font-size: 48px !important; }
            .blackboard { border: 4px solid #000 !important; padding: 15px !important; }
            .teacher-controls { display: none !important; }
        }
        </style>
    """, unsafe_allow_html=True)

    # --- 5. 자리배치 시각적 출력 (메모리 데이터 사용) ---
    ...
    if user['name'] in ["교사", "관리자"]:
        ...
        with st.expander("⚙️ 조건 설정 및 관리"):
            st.info(f"💡 전체 {total_seats}석 중 **정확히 {required_x_count}개의 'X'**를 선택해야 배치가 가능합니다.")
            
            # X석 선택 (화면과 일치하도록 5->1분단 순서로 표시)
            disabled_seats = []
        ...
        with c1: # 🎲 자리 셔플
            if st.button("🎲 자리 셔플", use_container_width=True):
                if len(disabled_seats) != required_x_count:
                    st.error(f"❌ X 개수가 맞지 않습니다! ({required_x_count}개 필요)")
                else:
                    with st.spinner("최적의 배치를 계산 중..."):
                        ...
                        for r in range(rows_count):
                            for c in range(columns_count):
                                if (r, c) in disabled_seats: new_grid[r][c] = "X"
                                elif s_idx < len(shuffled):
                                    new_grid[r][c] = shuffled[s_idx]
                                    s_idx += 1
                        st.session_state.current_layout = new_grid
                        st.toast("✅ 셔플 완료!")
                        st.rerun()
