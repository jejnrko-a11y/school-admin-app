...
def show_page(conn, user):
    ...
    if user['name'] in ['교사', '관리자']:
        with st.expander("🛠 교사용 제어 패널", expanded=True):
            ...
            if c1.button("▶ 투표 시작"): 
                conn.update(worksheet="선거상태", data=pd.DataFrame([{'상태': '진행중'}])); st.rerun()
            if c2.button("■ 투표 종료"): 
                with st.empty():
                    for i in range(5, 0, -1):
                        st.write(f"## 🏁 {i}초 후 종료...")
                        time.sleep(1)
                conn.update(worksheet="선거상태", data=pd.DataFrame([{'상태': '종료'}])); st.rerun()
            if c3.button("🔄 데이터 리셋"):
    ...
    chalk_content = "<h2>" + ("⏳ 투표 대기" if status=="대기" else "🗳️ 투표 진행중" if status=="진행중" else "🏆 최종 결과") + "</h2>"
    
    if status == "대기":
        for i, row in candidates.iterrows(): chalk_content += f"<p>✍️ 기호 {i+1}번. {row['이름']}</p>"
    elif status == "종료":
        for _, row in candidates.iterrows(): chalk_content += f"<p>{row['이름']} : {int(row['득표수'])}표</p>"
