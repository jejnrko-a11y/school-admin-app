...
def show_page(conn, user):
    ...
    # 1. 데이터 로드 및 초기화
    if 'elec_data' not in st.session_state or 'elec_state' not in st.session_state:
    ...
    if user['name'] in ['교사', '관리자']:
        with st.expander("🛠 교사용 제어 패널"):
            ...
            if c1.button("▶ 투표 시작"): st.session_state.elec_state.at[0, '상태']='진행중'; conn.update(worksheet="선거상태", data=st.session_state.elec_state); st.rerun()
            if c2.button("■ 투표 종료"): st.session_state.elec_state.at[0, '상태']='종료'; conn.update(worksheet="선거상태", data=st.session_state.elec_state); st.rerun()
            if c3.button("🔄 리셋"): 
                df_data[['역할', '투표완료여부', '득표수']] = ['일반', 'X', 0]; conn.update(worksheet="선거데이터", data=df_data); conn.update(worksheet="선거상태", data=pd.DataFrame([{'상태': '대기'}])); st.session_state.elec_state.at[0,'상태']='대기'; st.rerun()
            if c4.button("👥 투표자"): st.session_state.show_voters = not getattr(st.session_state, 'show_voters', False)

            if getattr(st.session_state, 'show_voters', False): st.dataframe(df_data[['번호', '이름', '투표완료여부', '득표수']].sort_values('번호'))
    ...
    # 3. 칠판 UI: 후보자 실시간 기호 부여 및 노출
    candidates = df_data[df_data['역할'] == '후보'].copy()
    # 후보 등록 순서대로 기호 부여
    ...
    if status == "대기":
        for _, row in candidates.iterrows(): chalk += f"<p>✍️ 기호 {int(row['기호'])}번. {row['이름']}</p>"
    elif status == "종료":
        if st.button("✨ 결과 확인"): st.session_state.show_results = True
        if getattr(st.session_state, 'show_results', False):
            for _, row in candidates.iterrows(): chalk += f"<p>기호 {int(row['기호'])}번 {row['이름']} : {int(row['득표수'])}표</p>"

    st.markdown(f'<div class="chalkboard">{chalk}</div>', unsafe_allow_html=True)

    # 4. 학생 투표 (정확한 행 업데이트 로직)
