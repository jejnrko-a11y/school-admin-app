...
def show_page(conn, user):
    ...
    if user['name'] in ['교사', '관리자']:
        with st.expander("🛠 교사용 제어 패널"):
            ...
            if c1.button("▶ 투표 시작"): 
                new_s = pd.DataFrame([{'상태': '진행중'}]); conn.update(worksheet="선거상태", data=new_s)
                st.session_state.elec_state = new_s; st.rerun()
            if c2.button("■ 투표 종료"): 
                new_s = pd.DataFrame([{'상태': '종료'}]); conn.update(worksheet="선거상태", data=new_s)
                st.session_state.elec_state = new_s; st.rerun()
            if c3.button("🔄 리셋"):
                df_data[['역할', '투표완료여부', '득표수']] = ['일반', 'X', 0]
                conn.update(worksheet="선거데이터", data=df_data); conn.update(worksheet="선거상태", data=pd.DataFrame([{'상태': '대기'}])); st.rerun()
            if c4.button("👥 투표자"): st.session_state.show_voters = not getattr(st.session_state, 'show_voters', False)

            if getattr(st.session_state, 'show_voters', False): st.dataframe(df_data[['이름', '투표완료여부']])
    ...
    if status == "대기":
        for _, row in candidates.iterrows(): chalk += f"<p>✍️ {row['이름']}</p>"
    elif status == "종료":
        ...
        if getattr(st.session_state, 'show_results', False):
            if not candidates.empty:
                ...
                else: chalk += "<br><h3>득표 데이터가 없습니다.</h3>"
            else: chalk += "<br><h3>등록된 후보가 없습니다.</h3>"
